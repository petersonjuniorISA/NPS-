...
/* Trajetoria dentro do proprio quadro do NPS: onde o indice esteve e onde a
  ...
  /** Os meses da série, que é o que a tela mostra. */
  function eixoMeses() {
    return Object.keys(S.nps.meses || {}).sort()
      .map(m => ({ mes: m, label: mesCurto(m), bloco: S.nps.meses[m], parcial: false }));
  }

  /** As semanas de um mês, que é o que o card flutuante mostra. */
  function eixoSemanas(mes) {
    return semanasNps(mes).map(sem => ({
      label: "S" + sem.semana + (sem.label ? " · " + sem.label : ""),
      bloco: sem, parcial: sem.respostas < MIN_SEMANA
    }));
  }

  /** Meses que têm quebra semanal — só eles entram no card flutuante. */
  function mesesComSemanaNps() {
    return Object.keys(S.nps.semanas || {}).filter(m => (S.nps.semanas[m] || []).length > 1).sort();
  }
  ...
  /* Séries de dimensão de um eixo qualquer — serve tanto para os meses na
     tela quanto para as semanas dentro do card flutuante. */
  function seriesDeDimensao(eixo, ancora) {
    return DIMENSOES.map((d, i) => {
      ...
      return {
        ...
        spanGaps: true,
        rotulo: { casas: 2, cor: cor, soUltimo: true, indice: ancora },
  ...
  /** Clique num mês da evolução por dimensão: as semanas daquele mês. */
  function popDimensoes(elementos, eixo) {
    if (!elementos || !elementos.length) return;
    const mes = (eixo[elementos[0].index] || {}).mes;
    const comSemana = mesesComSemanaNps();
    if (!mes || !comSemana.length) return;
    abrirPop({
      titulo: "Nota por dimensão, semana a semana",
      meses: comSemana,
      mes: comSemana.includes(mes) ? mes : comSemana[comSemana.length - 1],
      sub: m => "Semanas de " + mesLabel(m).toLowerCase() +
        " · trecho pontilhado é semana com menos de " + MIN_SEMANA + " respostas",
      nota: m => {
        const sem = semanasNps(m).filter(x => x.respostas < MIN_SEMANA);
        return sem.length ? "<b>" + sem.length + "</b> semana(s) com amostra curta — o ponto " +
          "serve de sinal, não de conclusão." : "";
      },
      config: m => {
        const eixoSem = eixoSemanas(m);
        const ds = seriesDeDimensao(eixoSem, eixoSem.length - 1);
        return { type: "line",
          data: { labels: eixoSem.map(p => p.label), datasets: ds },
          options: opcoes({ layout: { padding: { top: 18, right: 48 } },
            plugins: { legend: legenda(10) },
            scales: { y: Object.assign(faixa(ds.map(d => d.data), .12, 0, 5),
                        { grid: { color: C.grid } }),
                      x: { grid: { display: false } } } }) };
      }
    });
  ...
  /** As 5 especialidades de maior volume, no eixo que vier. */
  function seriesDeEspecialidade(eixo, escolhidas, ancora) {
    return escolhidas.map((nome, i) => {
      const cor = CORES_SERIE[i % CORES_SERIE.length];
      return {
        label: nome,
        data: eixo.map(p => {
        ...
        pointRadius: 4, pointBackgroundColor: cor, pointBorderColor: "#fff", pointBorderWidth: 1.5,
        spanGaps: true,
        rotulo: { casas: 1, cor: cor, soUltimo: true, indice: ancora },
        segment: { borderDash: ctx => eixo[ctx.p1DataIndex].parcial ? [5, 4] : undefined }
      };
    });
  }
  ...
  function grafEvolucaoEspecialidades() {
    ...
    const datasets = seriesDeEspecialidade(eixo, escolhidas, eixo.length - 1);

    grafico("chart-evo-esp", { type: "line",
      data: { labels: eixo.map(p => p.label), datasets },
      options: opcoes({
        onClick: (e, els) => popEspecialidades(els, eixo, escolhidas),
        plugins: { legend: legenda(10) },
        layout: { padding: { right: 48 } },
  ...
  /** Clique num mês da evolução por especialidade: as semanas daquele mês. */
  function popEspecialidades(elementos, eixo, escolhidas) {
    if (!elementos || !elementos.length) return;
    const mes = (eixo[elementos[0].index] || {}).mes;
    const comSemana = mesesComSemanaNps();
    if (!mes || !comSemana.length) return;
    abrirPop({
      titulo: "NPS por especialidade, semana a semana",
      meses: comSemana,
      mes: comSemana.includes(mes) ? mes : comSemana[comSemana.length - 1],
      sub: m => "Semanas de " + mesLabel(m).toLowerCase() +
        " · as mesmas 5 especialidades de maior volume",
      nota: m => {
        const sem = semanasNps(m).filter(x => x.respostas < MIN_SEMANA);
        return sem.length ? "<b>" + sem.length + "</b> semana(s) com amostra curta — numa " +
          "especialidade isso pode ser uma resposta só." : "";
      },
      config: m => {
        const eixoSem = eixoSemanas(m);
        const ds = seriesDeEspecialidade(eixoSem, escolhidas, eixoSem.length - 1);
        return { type: "line",
          data: { labels: eixoSem.map(p => p.label), datasets: ds },
          options: opcoes({ layout: { padding: { top: 18, right: 48 } },
            plugins: { legend: legenda(10) },
            scales: { y: Object.assign(faixa(ds.map(d => d.data), 8, -100, 100),
                        { grid: { color: C.grid } }),
                      x: { grid: { display: false } } } }) };
      }
    });
